...
export default function HomePage() {
  ...
  const start = (x, y) => { setIsDrawing(true); ctx.beginPath(); ctx.moveTo(x, y); };
  const drawLine = (x, y) => { if (isDrawing) { ctx.lineTo(x, y); ctx.stroke(); } };
  const stop = () => { setIsDrawing(false); finalizeDrawing(); placeInAR(); };
      ...
      <div id="drawingUI" style={styles.drawingUI}>
        <div className="controls" style={styles.controls}>
          <div className="row" style={styles.row}>
            ...
            <button onClick={() => { finalizeDrawing(); placeInAR(); }}>Draw → AR</button>
          </div>
          <div className="row" style={styles.row}>
        ...
        <canvas
          ...
          onMouseUp={stop}
          onMouseLeave={stop}
          onTouchStart={e => { const { x, y } = getTouch(e); start(x, y); e.preventDefault(); }}
          onTouchMove={e => { const { x, y } = getTouch(e); drawLine(x, y); e.preventDefault(); }}
          onTouchEnd={stop}
        />
      </div>
