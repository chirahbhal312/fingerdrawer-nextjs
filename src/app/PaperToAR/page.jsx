      ...
      <div className="h-[10vh] sm:h-[12.5vh] bg-[#2C2C2C] border-t border-[#3C3C3C] shadow-lg p-2 sm:p-4">
        <div className="flex items-center justify-center max-w-4xl mx-auto w-full h-full">
          <div className="flex items-center justify-evenly w-full max-w-2xl gap-2 sm:gap-4">
            <button className={`${buttonClass(tool === "brush")} flex items-center gap-1 sm:gap-2`} onClick={() => setTool("brush")}>
              <Brush className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />
              <span className="hidden md:inline text-xs sm:text-sm">Brush</span>
            </button>
            <button className={`${buttonClass(showColorPopover)} flex items-center gap-1 sm:gap-2`} onClick={() => setShowColorPopover(!showColorPopover)}>
              <Palette className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />
              <div className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5 rounded border border-white shadow-sm" style={{ backgroundColor: brushColor }} />
            ...
            <button className={`${buttonClass(tool === "eraser")} flex items-center gap-1 sm:gap-2`} onClick={() => setTool("eraser")}>
              <Eraser className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" />
              <span className="hidden md:inline text-xs sm:text-sm">Eraser</span>
            </button>
            <button className={`${buttonClass(showSettingsPopover)} flex items-center gap-1 sm:gap-2`} onClick={() => setShowSettingsPopover(!showSettingsPopover)}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="modal-content bg-[#2C2C2C] rounded-2xl p-4 sm:p-6 max-w-xs sm:max-w-md w-full mx-4 shadow-2xl border border-[#3C3C3C]">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowColorPopover(false)} className="p-1 hover:bg-[#3C3C3C] rounded-full transition-colors">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 sm:gap-3">
                <input type="color" value={customColor} onChange={handleCustomColorChange} className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl border border-[#4C4C4C] cursor-pointer bg-[#3C3C3C]" />
                <input
                  type="text"
                  value={customColor}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="modal-content bg-[#2C2C2C] rounded-2xl p-4 sm:p-6 max-w-xs sm:max-w-md w-full mx-4 shadow-2xl border border-[#3C3C3C]">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowSettingsPopover(false)} className="p-1 hover:bg-[#3C3C3C] rounded-full transition-colors">
                <X className="w-6 h-6 text-[#CCCCCC]" />
              </button>
            </div>
            <div className="space-y-4">
              <div>
                ...
                <input type="range" min="1" max="50" value={brushSize} onChange={(e) => setBrushSize(parseInt(e.target.value))} className="w-full h-2 bg-[#3C3C3C] rounded-lg appearance-none cursor-pointer slider" />
              </div>
              <div className="flex items-center justify-center py-4">
                <div className="bg-[#3C3C3C] rounded-xl p-4">
                  <div className="rounded-full" style={{ width: `${Math.max(brushSize, 4)}px`, height: `${Math.max(brushSize, 4)}px`, backgroundColor: brushColor }} />
                </div>
              </div>
              <div className="grid grid-cols-4 gap-2">
                {[2, 5, 10, 20].map((size) => (
                  <button key={size} onClick={() => setBrushSize(size)} className={`py-2 px-3 rounded-xl text-sm font-medium transition-colors ${
                    brushSize === size ? "bg-[#7C3AED] text-white" : "bg-[#3C3C3C] text-[#CCCCCC] hover:bg-[#4C4C4C] hover:text-white"
                  }`}>
                    {size}px
                  </button>
                ))}
